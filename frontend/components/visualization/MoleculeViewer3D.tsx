    ...
    <div className="relative w-full bg-[#010409] rounded-3xl border border-white/5 overflow-hidden group" style={{ height }}>
      ...
      <div className="absolute top-8 left-8 z-20 flex flex-col gap-3 pointer-events-none">
        <div className="flex items-center gap-4">
          <div className="w-2.5 h-2.5 rounded-full bg-emerald shadow-[0_0_15px_rgba(16,185,129,0.8)] animate-pulse" />
          <span className="text-[13px] font-black uppercase tracking-[0.5em] text-white/80">Active Scanner</span>
        </div>
        <div className="text-[14px] font-mono text-emerald font-black tracking-[0.2em] uppercase">
          SDF-3D STRM: {smiles}
        </div>
      </div>
      ...
      {loading && (
        <div className="absolute inset-0 z-30 flex items-center justify-center bg-void/80 backdrop-blur-sm">
          ...
          <div className="w-20 h-20 border-4 border-emerald/10 border-t-emerald rounded-full animate-spin shadow-[0_0_40px_rgba(16,185,129,0.2)]" />
          <div className="text-[14px] font-black uppercase tracking-[0.5em] text-emerald drop-shadow-2xl">Materializing Structure...</div>
        </div>
        </div>
      )}
...
function StyleButton({ active, onClick, icon, label }: { active: boolean; onClick: () => void; icon: React.ReactNode; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-4 px-6 py-3 rounded-2xl transition-all ${
        active 
        ? 'bg-emerald text-void font-black shadow-[0_0_30px_rgba(16,185,129,0.5)]' 
